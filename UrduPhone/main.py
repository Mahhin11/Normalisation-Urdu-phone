...
def get_wordslist():
    with open("./Output Files/Default - Word List.txt", "r", encoding='utf-8') as ff:
        text = ff.read().split()
    words = text
    
    with open("./Input Files/Gold Standard.txt", "r", encoding='utf-8') as ff:
        text = ff.read().split()
    strwords = []
    for t in text:
...
def get_words_and_write_UrduPhone():
    ...
    UrduPhone_ids = {key: idx for idx, key in enumerate(UrduPhone_dict.keys())}

    with open("./Output Files/Default - UrduPhone - ID.txt", "w", encoding='utf-8') as ff, \
         open("./Output Files/Default - UrduPhone.txt", "w", encoding='utf-8') as f2:
        for w in words:
            UrduPhone = phonetics.UrduPhone(w)
            ff.write(f"{w},{UrduPhone},{UrduPhone_ids[UrduPhone]}\n")
            f2.write(f"{w},{UrduPhone}\n")
def getText():
    with open("./Input Files/Dataset.txt", "r", encoding='utf-8') as ff:
        text = ff.read().lower()
    return text
...
def _5PrevList():
	...
	text = ff.read() ; text = text.split("\n")
	ff.close()
	ff=open("./Output Files/Default - 5 Prev Word ID List.txt","w")
...
def PreProcess5Files():
    with open("./Output Files/Default - 5 Prev Word ID List.txt", "r", encoding='utf-8') as ff:
        text = ff.read().split("\n")
    with open("./Output Files/Default - 5 Prev Word ID List.txt", "w", encoding='utf-8') as ff:
        for t in text:
            if t.find(",") != -1:
                ff.write(t + "\n")

    with open("./Output Files/Default - 5 Next Word ID List.txt", "r", encoding='utf-8') as ff:
        text = ff.read().split("\n")
    with open("./Output Files/Default - 5 Next Word ID List.txt", "w", encoding='utf-8') as ff:
        for t in text:
            if t.find(",") != -1:
                ff.write(t + "\n")
def getUrduPhoneIds():
...
def PreProcess5FilesForUrduPhone():
    ...
    with open("./Output Files/Default - 5 Prev Word ID List.txt", "r", encoding='utf-8') as ff:
        text = ff.read().split("\n")
    with open("./Output Files/Default - 5 Prev UrduPhone ID List.txt", "w", encoding='utf-8') as ff:
        for t in text:
            words = t.split(",")
            ff.write(words[0])
    ...
    with open("./Output Files/Default - 5 Next Word ID List.txt", "r", encoding='utf-8') as ff:
        text = ff.read().split("\n")
    with open("./Output Files/Default - 5 Next UrduPhone ID List.txt", "w", encoding='utf-8') as ff:
        for t in text:
            words = t.split(",")
            ff.write(words[0])
            for i in range(1, len(words)):
